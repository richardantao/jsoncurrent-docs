...
You can provide a `flush` callback to pace `change` event emission.
...
When `flush` is provided:

- only `change` event emission is paced
- `pathstart`, `pathcomplete`, and `error` remain immediate
- middleware runs on queued chunks before patch application
- queued `change` events are drained before stream-level `complete`
...
```typescript
collector.on('change', (state: Partial<T>, path: string, op: Op) => {
  setData(state)
})
```

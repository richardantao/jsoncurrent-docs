...
| `flush` | `FlushFn` | Optional pacing callback passed to the internal `Collector` to pace `change` event emission. |
...
**Flush behavior.** When `flush` is provided, only `change` emission is paced. `pathstart`,
`pathcomplete`, and `error` remain immediate, and queued `change` events are drained before
stream-level `complete`.
